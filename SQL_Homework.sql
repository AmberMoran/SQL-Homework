...
WHERE c.country IN ('Afghanistan'
	,'Bangladesh'
    ,'China');
...
SELECT CONCAT(cu.first_name," ",cu.last_name) AS FullName
,email
,c.country
...
SELECT 
c.name
,CONCAT('$', FORMAT(SUM(p.amount), 2)) AS GrossRevenue
...
CREATE VIEW Top5GrossRevenuebyGenre AS
SELECT 
c.name
,CONCAT('$', FORMAT(SUM(p.amount), 2)) AS GrossRevenue
...
SELECT * FROM top5grossrevenuebygenre;

-- 8c. You find that you no longer need the view `top_five_genres`. Write a query to delete it.

DROP VIEW top5grossrevenuebygenre;
